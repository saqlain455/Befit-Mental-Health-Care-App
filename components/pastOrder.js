 ...
 */
 ...
 function PastOrder({ navigation }) {
   ...
   const getappointment = async () => {
     ...
     fetch(
       "http://10.113.61.200:3000/patient/ViewOrder/" + id,
       requestOptions
     )
   ...
   async function fiterApoinement() {
       const ap=await appoinementdata.filter((item)=>item.status==="completed")
       ...
       <DataTable>
         <DataTable.Header>
           ...
           {filterAppointment.map((item,index) => {
            ...
            <DataTable.Cell ><Text style={{color:'green',fontWeight:'bold'}}> Completed</Text></DataTable.Cell>
